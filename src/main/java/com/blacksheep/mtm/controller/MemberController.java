...
public class MemberController {
    ...
    @Autowired
    private MemberFllowService memberFllowService;
    ...
    public DataTablesResult getMemberList(int draw, int start, int length, String searchKey,
        ...
        //获取客户端需要排序的列
        String[] cols = {"checkbox","id", "username","sex", "phone", "email", "created", "updated", "state"};
    ...
    @RequestMapping(value = "/member/fllow/{id}",method = RequestMethod.GET)
    @ApiOperation(value = "根据用户id得到跟进数据")
    public DataTablesResult getMemberFllowList(@PathVariable Integer id,int draw, int start, int length,
                                               @RequestParam("search[value]") String search,
                                               @RequestParam("order[0][column]") int orderCol,
                                               @RequestParam("order[0][dir]") String orderDir){

        String[] cols = {"checkbox","id", "fllow_type","fllow_content", "fllow_time", "create_time", "update_time"};
        String orderColumn = cols[orderCol];
        //默认排序列
        if(orderColumn == null) {
        ...
        DataTablesResult result=memberFllowService.getListByUser(id,draw,start,length,orderColumn,orderDir);
        return result;
    }

    @RequestMapping(value = "/member/fllow/list",method = RequestMethod.GET)
    @ApiOperation(value = "根据用户id得到跟进数据")
    public DataTablesResult getMemberFllowList(int draw, int start, int length, String searchKey,
                                               String minDate, String maxDate,
                                               @RequestParam("search[value]") String search,
                                               @RequestParam("order[0][column]") int orderCol,
                                               @RequestParam("order[0][dir]") String orderDir){
        ...
        DataTablesResult result = memberFllowService.getAllList(draw,start,length,searchKey,minDate,maxDate,orderColumn,orderDir);
        return result;
    }
}
