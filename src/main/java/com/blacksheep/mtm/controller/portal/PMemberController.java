...
import com.blacksheep.mtm.common.utils.GeetestLib;
import com.blacksheep.mtm.common.utils.ResultUtil;
import com.blacksheep.mtm.dto.front.CommonDto;
...
public class PMemberController {
    ...
    @Autowired
    private SSORegisterService registerService;
    @Autowired
    ...
    public Result<Object> register(@RequestBody MemberLoginRegist memberLoginRegist,
        ...
        if (gtResult == 1) {
            // 验证成功
            int result = registerService.register(memberLoginRegist);
            if (result == 0) {
                return new ResultUtil<Object>().setErrorMsg("该手机号已被注册");
            } else if (result == -1) {
                return new ResultUtil<Object>().setErrorMsg("用户名手机不能为空");
            }
